...
resource "camc_scriptpackage" "get_cluster_config" {
	depends_on = ["null_resource.config-output-scripts"]
  	program = "${split(",", length(var.openshift_cluster_name) > 0 ? join(",", list("sudo /bin/bash /tmp/gather_output.sh -c ${var.openshift_cluster_name} -as ${var.openshift_server} -ap ${var.openshift_port} -u ${var.openshift_user}")) : join(",", list("sudo /bin/bash /tmp/gather_output.sh -as ${var.openshift_server} -ap ${var.openshift_port} -u ${var.openshift_user}")))}"
  	on_create = true
