...
do
    sshpass -p $os_password ssh-copy-id -i ~/.ssh/id_rsa.pub -o StrictHostKeyChecking=no ${nodehostnames[index]}.$domain_name
done
...
for index in "${!nodehostnames[@]}"
do
    echo "${nodehostnames[index]}.$domain_name openshift_node_group_name='node-config-compute'" >> /etc/ansible/hosts
done
if grep -q "[glusterfs]" /etc/ansible/hosts; then
    disk=$(head -n 1 /tmp/glusterfs_disk.txt)
    for index in "${!nodehostnames[@]}"
    do
        sed -i -e "/^\[glusterfs\]/a ${nodehostnames[index]}.$domain_name glusterfs_devices='[ \"$disk\" ]'" /etc/ansible/hosts
    done
fi
...
if ansible-playbook playbooks/openshift-node/scaleup.yml; then
    ...
    do
        ...
        sed -i -e "/^\[new\_nodes\]/i ${nodehostnames[index]}.$domain_name openshift_node_group_name='node-config-compute'" /etc/ansible/hosts
        sed -i -e "/^\[glusterfs\]/a ${nodehostnames[index]}.$domain_name glusterfs_devices='[ \"$disk\" ]'" /etc/ansible/hosts
    done
else
