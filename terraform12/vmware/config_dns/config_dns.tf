...
resource "null_resource" "setup_dhcp" {
  depends_on = [
    null_resource.master_dependsOn,
    null_resource.setup_dns_server,
  ]
...
resource "null_resource" "add_worker_node_dns_record" {
  depends_on = [
    ...
    null_resource.setup_dns_server,
    null_resource.setup_dhcp,
    null_resource.add_master_node_dns_record,
  ]
...
resource "null_resource" "finish_config_dns" {
  depends_on = [
    ...
    null_resource.setup_dhcp,
    null_resource.add_master_node_dns_record,
    null_resource.add_worker_node_dns_record,
  ]
  provisioner "local-exec" {
